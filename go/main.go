...
func GetTransactionEvidencesByItemIds(q sqlx.Queryer, itemIds []int64) []TransactionEvidence {
	inQuery, inArgs, _ := sqlx.In("SELECT * FROM `transaction_evidences` WHERE `item_id` in (?)", itemIds)

	evidences := []TransactionEvidence{}
	err := sqlx.Select(q, &evidences, inQuery, inArgs...)
	if err != nil {
		return nil
	}
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	wg := &sync.WaitGroup{}
	a := sync.Mutex{}
	statusMap := map[string]*APIShipmentStatusRes{}
	for _, shipping := range shippings {
		...
		go func() {
			...
			if shipping.TransactionEvidenceID != 0 {
				ssr, _ := APIShipmentStatus(getShipmentServiceURL(), &APIShipmentStatusReq{
					ReserveID: shipping.ReserveID,
				})

				a.Lock()
				statusMap[shipping.ReserveID] = ssr
				a.Unlock()
			}
		}()
	}
	...
	for _, item := range items {
		...
		var transactionEvidence TransactionEvidence
		for _, e := range evidences {
			if item.ID == e.ItemID {
				transactionEvidence = e
				break
			}
		}
		...
		if transactionEvidence.ID > 0 {
			...
			itemDetail.ShippingStatus = statusMap[shipping.ReserveID].Status
		}
